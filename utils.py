...
def count_parameters(model):
    ...
    for name, parameter in model.named_parameters():
        ...
        shape = param.shape
        table.add_row([name, shape, param])
        total_params+=param
    print(table)
    print(f"Total Trainable Params: {total_params}")
    return total_params

def df2Tensor(df_x, df_y, x_column, X_dtype, Y_dtype):
    """
    ...
    """
    X = df_x[x_column].values.tolist()
    X = t.from_numpy(np.array(X)).type(X_dtype)
    Y = t.from_numpy(df_y.values.astype(Y_dtype))
    return X, Y


def make_random_classification_data(N_train, N_test, C, H, W, dtype, N_classes):
    """
    ...
    """
    # conv takes batches in NCHW
    X_train = t.rand((N_train, C, H, W), dtype=dtype)
    X_test = t.rand((N_test, C, H, W), dtype=dtype)

    Y_train = t.randint(0, N_classes, (N_train,1))
    Y_test = t.randint(0, N_classes, (N_test,1))
    return X_train, X_test, Y_train, Y_test


def make_random_synthesis_data(N, C, H, W, dtype):
    # conv takes batches in NCHW
    X_train = t.rand((N, C, H, W), dtype=dtype)
...
# df_perf = pd.read_csv('crit_diff_example_data.csv',index_col=False)
